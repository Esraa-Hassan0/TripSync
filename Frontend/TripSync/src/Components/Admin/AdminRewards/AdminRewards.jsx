...
function AdminRewards({ all_rewards, userId, rerender }) {
  ...
  const generalRewards = all_rewards.filter(
    (reward) => reward.type === "general"
  ).length;
  const promotionRewards = all_rewards.filter(
    (reward) => reward.type === "promotion"
  ).length;
  const freeTripRewards = all_rewards.filter(
  ...
  const DeleteReward = async (reward_id) => {
    const reward = await axios
      .delete(
        `https://backendtripsync.vercel.app/api/v1/rewards/deleteReward/${reward_id}`,
        {
          headers: {
